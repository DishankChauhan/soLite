...
async function sendTokens(
  ...
  try {
    ...
    // Queue transaction notification for sender
    ...
    // Also notify recipient if they have an account in our system
    try {
      ...
      if (recipientWalletResult.rows.length > 0) {
        const recipientUserId = recipientWalletResult.rows[0].user_id;
        await notificationService.queueTransactionNotification(
          recipientUserId,
          'RECEIVE',
          amount,
          normalizedTokenType,
          wallet.publicKey // sender's address
        );
        logger.info(`Queued RECEIVE notification for recipient user ${recipientUserId}`);
      }
    } catch (error) {
      logger.warn(`Could not notify recipient: ${error}`);
      // Don't fail the transaction if recipient notification fails
    }
